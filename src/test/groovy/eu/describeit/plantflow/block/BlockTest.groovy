...
class BlockTest extends Specification {
  ...
  def "getId concatenates type and idx"() {
    given:
    def node = new Block(type: BlockType.ROOT_BLOCK, idx: 2)
  ...
  def "addChildren appends the child to children list"() {
    given:
    def parent = new Block(type: BlockType.ROOT_BLOCK)
    def child  = new Block(type: BlockType.LOOP, idx: 1)
  ...
  def "find(BlockType) returns all nodes of the given type recursively"() {
    given:
    def root = new Block(type: BlockType.ROOT_BLOCK)
    def fork = new Block(type: BlockType.FORK, idx: 7)
    def loop = new Block(type: BlockType.LOOP, idx: 3)
    def inLoop1 = new Block(type: BlockType.FORK_BLOCK, idx: 4)
    def inLoop2 = new Block(type: BlockType.FORK_BLOCK, idx: 5)
  ...
  def "isFinished is true only if all non-ACTION children have empty nextActions"() {
    given:
    Block parent = new Block(type: BlockType.ROOT_BLOCK)
    def nonActionEmpty = new Block(type: BlockType.LOOP, idx: 1)
    def nonActionHasNext = new Block(type: BlockType.CONDITIONAL, idx: 2)
    def actionChild = new Block(type: BlockType.ACTION, name: 'act')
